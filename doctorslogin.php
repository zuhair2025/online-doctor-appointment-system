...
if(isset($_POST['login'])){
   ...
   $sql="SELECT * FROM tbl_doctor WHERE doctorEmail='$email' AND doctorPassword='$password'";
    if(mysqli_query($conn,$sql)){
            $query_result=mysqli_query($conn,$sql);
            $user_info=mysqli_fetch_assoc($query_result);
            if($user_info){
                //session_start();
                $_SESSION['doctorName']=$user_info['doctorName'];
                $_SESSION['doctorId']=$user_info['doctorId'];
