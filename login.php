...
if(isset($_POST['login'])){
   ...
   $sql="SELECT * FROM tbl_admin WHERE adminEmail='$email' AND adminPassword='$password'";
    if(mysqli_query($conn,$sql)){
            $query_result=mysqli_query($conn,$sql);
            $user_info=mysqli_fetch_assoc($query_result);
            if($user_info){
                //session_start();
                $_SESSION['adminName']=$user_info['adminName'];
                $_SESSION['adminId']=$user_info['adminId'];
                $_SESSION['adminPosition']=$user_info['adminPosition'];
                header('Location:main/dashboard.php');
            }else{
...
<body style="background: url(main/images/library.jpg) no-repeat center center fixed; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover; ">
  ...
  <form class="login-container" id="loginFrom" method="POST">
    ...
    <p><input type="captcha" name="captcha" placeholder="captcha..."></p>
